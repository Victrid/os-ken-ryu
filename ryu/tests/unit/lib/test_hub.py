...
class TestHubType(unittest.TestCase):
    ...
    def test_eventlet_mode(self):
        hub = importlib.reload(ryu.lib.hub)
    ...
    def test_other_mode(self):
        self.assertRaises(NotImplementedError, importlib.reload, ryu.lib.hub)


class TestStreamServerEventlet(unittest.TestCase):
    @mock.patch.dict(os.environ, {"OSKEN_HUB_TYPE": "eventlet"})
    def setUp(self):
        self.hub = importlib.reload(ryu.lib.hub)

    @mock.patch("ryu.lib.ip.valid_ipv6", return_value=False)
    @mock.patch("ryu.lib.hub.listen")
    ...
    @mock.patch("ryu.lib.ip.valid_ipv6", return_value=True)
    @mock.patch("ryu.lib.hub.listen")
...
class TestThreadManagementEventlet(unittest.TestCase):
    ...
    @mock.patch('ryu.lib.hub.LOG.error')
